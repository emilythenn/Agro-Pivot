...
import { Cloud, Droplets, Thermometer, Wind, CloudRain, Sun, AlertTriangle, RefreshCw, Loader2, MapPin } from "lucide-react";
...
export default function WeatherPage() {
  ...
  const loadWeather = async (district?: string) => {
    ...
    setLoading(true);
    try {
      const data = await fetchWeatherData(d, language, user?.id, farmContext);
  ...
  // Load weather when district is set or changes from profile
  useEffect(() => {
    if (selectedDistrict) loadWeather();
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-8">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button variant="outline" size="sm" onClick={() => loadWeather()} className="gap-2 self-start">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
        <GlassCard className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] text-muted-foreground/60 ml-2">(synced from your profile: {profile.district}, {profile.state})</span>
          </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div>
            <h3 className="text-base font-semibold text-foreground mb-4">7-Day Forecast — {selectedDistrict}, {selectedState}</h3>
          ...
          <p className="text-[10px] text-muted-foreground/50 text-right">AI-Generated Weather Intelligence • {selectedDistrict}, {selectedState}</p>
        </>
      )}
    </div>
  );
